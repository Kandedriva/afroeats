...
// ============================================
// GROCERY STORE ORDERS MANAGEMENT
// ============================================

/**
 * GET /api/owners/grocery-orders
 * Get all grocery orders for the store owner (if they have a grocery store)
 * NOTE: Temporarily unprotected for testing - re-add requireOwnerAuth when store owner registration is ready
 */
router.get("/grocery-orders", async (req, res) => {
  try {
    ...
    const ordersResult = await pool.query(`
      ...
      WHERE go.status IN ('paid', 'preparing', 'ready', 'out_for_delivery', 'delivered')
      ORDER BY go.created_at DESC
    `);

    console.log(`📦 Found ${ordersResult.rows.length} grocery orders`);
    ...
    res.status(500).json({ error: "Failed to load grocery orders", details: error.message });
  }
});

/**
 * PATCH /api/owners/grocery-orders/:id/complete
 * Mark a grocery order as completed/delivered
 * NOTE: Temporarily unprotected for testing - re-add requireOwnerAuth when store owner registration is ready
 */
router.patch("/grocery-orders/:id/complete", async (req, res) => {
  try {
    ...
    // TODO: Send notification to customer about delivery
    ...
    console.error("Failed to complete grocery order:", error);
    res.status(500).json({ error: "Failed to complete order" });
  }
});
