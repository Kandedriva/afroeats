...
function GroceryOwnerVerifyEmail() {
  ...
  useEffect(() => {
    if (!email) {
      navigate('/grocery-owner/register');
    }
  }, [email, navigate]);
  ...
  const handleVerify = async (e) => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/grocery-owners/verify-email`, {
        method: 'POST',
  ...
  const handleResend = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/grocery-owners/resend-verification`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-lg shadow-xl p-8">
          ...
          <div className="mt-6 text-center border-t pt-4">
            <Link to="/grocery-owner/register" className="text-sm text-gray-600 hover:text-gray-800">
...
export default GroceryOwnerVerifyEmail;
